...
import { getScanLink, shortenAddress } from '../../utils'
...
const WarningContainer = styled.div`
  ...
  width: 100%;
  padding: 1rem;
  background: ${({ theme }) => transparentize(0.95, theme.yellow2)};
        ...
        <AutoColumn gap="10px" justify="flex-start">
          ...
          {chainId && (
            <ExternalLink style={{ fontWeight: 400 }} href={getScanLink(chainId, token.address, 'token')}>
              <TYPE.blue title={token.address}>{shortenAddress(token.address)} (View on Polygonscan)</TYPE.blue>
            </ExternalLink>
          )}
        </AutoColumn>
      </AutoRow>
      ...
      <WarningContainer className="token-warning-container">
        <AutoColumn gap="lg">
          ...
          <TYPE.body color={'red2'}>
            Anyone can create an ERC20 token with <em>any</em> name, including creating fake versions of existing tokens
